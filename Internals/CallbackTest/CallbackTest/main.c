...
static void update_callback (csqldb *db, const char *table_name, int op, int64 rowid, void *data) {
    printf("notification_callback %s %d %lld\n", table_name, op, rowid);
    if (op == 9) return;
    ...
    char sql[512];
    snprintf(sql, sizeof(sql), "SELECT * FROM %s WHERE rowID=%lld;", table_name, rowid);
    csqlc *c = cubesql_select(db, sql, kFALSE);
    if (c != NULL) print_cursor(c);
}
...
static int do_setup (csqldb *db) {
    ...
    err = cubesql_set_update_callback(db, "foo", update_callback, NULL);
    if (err != CUBESQL_NOERR) goto abort;
...
int main (void) {
	...
	if (cubesql_connect(&db, HOSTNAME, CUBESQL_DEFAULT_PORT, USERNAME, PASSWORD, CUBESQL_DEFAULT_TIMEOUT, CUBESQL_ENCRYPTION_AES256) != CUBESQL_NOERR) goto abort;
    
    // setup database and table
    if (do_setup(db) == 0) goto abort;
